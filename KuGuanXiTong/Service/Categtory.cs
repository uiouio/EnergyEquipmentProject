...
namespace KuGuanXiTong.Service
{
    ...
    class Categtory : BaseService
    {
        ...
        */
        ...
        public IList GetBrokenpart(string code, string name, string per, long type, long t1, long t2)
        {

            IList i = null;
            string sql = "from BrokenParts u where u.State = "
                + (int)BaseEntity.stateEnum.Normal +
                " and u.ReplyTime>" + t1 +
                " and u.ReplyTime<" + t2 +
                " and u.GoodsBaseInfoId.GoodsClassCode like '%" + code + "%'" +
                " and u.BrokenType = " + type +
                " and u.ReplyPersonId.Name like '%" + per + "%'" +
                " and u.GoodsBaseInfoId.GoodsName like '%" + name + "%' order by u.ReplyTime DESC";

            string sql2 = "from BrokenParts u where u.State = "
                 + (int)BaseEntity.stateEnum.Normal +
                 " and u.ReplyTime>" + t1 +
                 " and u.ReplyTime<" + t2 +
                 " and u.GoodsBaseInfoId.GoodsClassCode like '%" + code + "%'" +
